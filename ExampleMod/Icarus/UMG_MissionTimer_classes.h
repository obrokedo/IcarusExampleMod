...
struct UUMG_MissionTimer_C : UUserWidget {
	...
	bool RecheckVisibility; // 0x300(0x01)
	char pad_301[0x3]; // 0x301(0x03)
	int32_t LastUpdateValue; // 0x304(0x04)

	void SetTime(struct TArray<struct FText>& Time); // Function UMG_MissionTimer.UMG_MissionTimer_C.SetTime // (Public|HasOutParms|HasDefaults|BlueprintCallable|BlueprintEvent) // @ game+0x1be0000
	void Tick(struct FGeometry MyGeometry, float InDeltaTime); // Function UMG_MissionTimer.UMG_MissionTimer_C.Tick // (BlueprintCosmetic|Event|Public|BlueprintEvent) // @ game+0x1be0000
	void ExecuteUbergraph_UMG_MissionTimer(int32_t EntryPoint); // Function UMG_MissionTimer.UMG_MissionTimer_C.ExecuteUbergraph_UMG_MissionTimer // (Final|UbergraphFunction|HasDefaults) // @ game+0x1be0000
};
